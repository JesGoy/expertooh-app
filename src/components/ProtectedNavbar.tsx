...
export default function ProtectedNavbar({ username }: { username?: string }) {
  return (
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <nav className="hidden sm:flex items-center gap-4 ml-4 text-sm">
            <Link className="text-neutral-600 hover:text-ink" href="/dashboard">Dashboard</Link>
            <Link className="text-neutral-600 hover:text-ink" href="/reports">Reportes</Link>
          </nav>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-xs text-neutral-500 hidden sm:inline">{username || 'Usuario'}</span>
